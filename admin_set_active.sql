-- ============================================================
-- SalesPlace — تفعيل الدالة الناقصة وراء زر "إيقاف/تفعيل" بشاشة إدارة المستخدمين.
-- الواجهة (زر الإيقاف، شارة "موقوف/مفعّل") كانت موجودة فعلاً بالتطبيق، لكن الدالة
-- admin_set_active التي يستدعيها الزر لم تكن قد أُضيفت لقاعدة البيانات فعلياً —
-- الضغط على الزر كان يفشل بصمت (الاستدعاء يرجع خطأ "دالة غير موجودة").
-- إضافة دالة واحدة فقط — لا تمسّ أي جدول أو دالة موجودة (عمود is_active على app_users
-- موجود أصلاً ويُستخدم في كل دوال الإدارة الأخرى عبر coalesce(is_active,true)).
